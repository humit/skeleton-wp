...
# Specify W3TC Access Key and Secret Key if set
if ! [ -z "${CDN_S3_KEY}" ] && ! [ -z "${CDN_S3_SECRET}" ] && ! [ -z "${CDN_S3_BUCKET}" ] && ! [ -z "${CDN_S3_REGION}" ] && [ -f /var/www/html/wp-content/w3tc-config/master.php  ];then
 echo "Filling in S3 CDN credentials"
 sed -i -e 's@__CDN_S3_KEY__@'"$CDN_S3_KEY"'@' -e 's@__CDN_S3_SECRET__@'"$CDN_S3_SECRET"'@' /var/www/html/wp-content/w3tc-config/master.php
 sed -i -e 's@__CDN_S3_BUCKET__@'"$CDN_S3_BUCKET"'@' -e 's@__CDN_S3_REGION__@'"$CDN_S3_REGION"'@' /var/www/html/wp-content/w3tc-config/master.php
else
 echo "Cannot find S3 CDN credentials or w3tc/master.php"
fi
